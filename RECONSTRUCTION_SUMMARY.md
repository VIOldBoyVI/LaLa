...
1. **Отсутствующий элемент**: В файле `/workspace/templates/index.html` JavaScript код ссылается на элемент с ID `currentPlayerSelect`, которого не существует в HTML-разметке.
2. **Незавершенная интеграция API**: Несмотря на наличие API-эндпоинтов в backend, frontend не использует их для получения и проверки вопросов.
3. **Инconsistent логика**: В разных шаблонах реализована разная логика игры.
