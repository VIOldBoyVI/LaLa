...
Символы, отображаемые на игровом поле, можно настроить в файле `/workspace/config.py`. Для этого измените массив `SYMBOLS` на желаемые символы:

```python
SYMBOLS = ['*', '+', '%', '@', '#', '&', '=', '~', '^', '!']  # Friendlier symbols
```

После изменения конфигурации перезапустите приложение, чтобы изменения вступили в силу.
